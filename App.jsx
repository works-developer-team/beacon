...
import { View, StyleSheet, PermissionsAndroid, Platform, Text, ScrollView } from 'react-native';
...
const BLEWebView = () => {
  ...
  const parseIBeaconData = (device) => {
    ...
    if (!manufacturerData || !manufacturerData.bytes) {
      console.log('❌ manufacturerData 없음:', device.advertising.manufacturerData);
      return { uuid: 'N/A', major: 'N/A', minor: 'N/A' };
    }
    ...
    if (bytes.length >= 23) {
      ...
      const uuid = uuidParts.map(part => part.map(b => b.toString(16).padStart(2, '0')).join('')).join('-');
      ...
      setTimeout(async () => {
        try {
          ...
          peripherals.forEach(device => {
            console.log('📡 발견된 장치 상세 정보:', JSON.stringify(device, null, 2));
          });

          const giworksDevices = peripherals
            .filter(device => device.name && device.name.startsWith('GIWORKS'))
            .map(device => {
          ...
          // 📌 WebView로 데이터 전달 (onReceiveBLEData가 존재하는지 체크 후 실행)
          if (webViewRef.current) {
            webViewRef.current.injectJavaScript(`
              if (typeof window.onReceiveBLEData === 'function') {
                window.onReceiveBLEData(${JSON.stringify(giworksDevices)});
              }
            `);
          }
        } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.deviceList}>
        {devices.length === 0 ? (
          <Text style={styles.noDeviceText}>🔍 감지된 GIWORKS 비콘이 없습니다.</Text>
        ) : (
